...
@php $que = DB::table('fmt_fillupmulti_ques')->where('id', $message)->first(); @endphp
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.fillupmulti.update', $que->id)}}" method="post" class="fmt_box">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    <div class="flex flex-wrap -mx-4 my-4">{{-- flex-wrap --}}
                        <div class="w-full px-4">{{-- w-1/3 --}}
                            <div class="my-2">
                                ...
                                <textarea class="w-full border border-gray-500 rounded-lg p-2" name="question_title" id="" cols="30" rows="4" placeholder="Question" required>{{$que->question_title}}</textarea>
                            </div>
                        </div>{{-- //w-1/3 --}}
                    </div>{{-- //flex-wrap --}}
                    <div class="my-2">
                    ...
                    @php $answers = DB::table('fmt_fillupmulti_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_fillupmulti_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
                        <div class="flex flex-wrap my-4" id="ans1">
                            <div class="w-full">
                                ...
                                <input class="my-2 border border-gray-500 p-2 w-full rounded-lg" type="text" name="answer{{$answer->id}}" placeholder="Answer" value="{{$answer->answer_title}}">
                            </div>
